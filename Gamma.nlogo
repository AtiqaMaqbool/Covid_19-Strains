...
to setup-turtles
  create-turtles init-population [
    ...
    set exposed? false
    set days-exposed 0
    set vaccinated? false ;; Initialize vaccinated status
    setxy random-pxcor random-pycor
  ]
end
to go
  tick
  ask turtles [rt random 100 lt random 100 fd 1]
  if (count turtles with [infected?] = 0) or (count turtles with [infected?] = init-population)
  [stop]
...
to vaccinate-turtles
  ask n-of vaccination-rate turtles with [not infected? and not immune? and not vaccinated?] [
    set vaccinated? true
    set immune? true
    set color green
  ]
...
to infect-susceptibles ;; S -> I
  ask turtles [
    ...
    if (random-float 1 <  1 - (((1 - transmission.rate) ^ infected-neighbors)) and not immune? and not vaccinated?)
    [set infected? true]
  ]
end
...
to recolor
  ask turtles with [infected?] [set color red]
  ask turtles with [immune? and vaccinated?] [set color green]
  ask turtles with [immune? and not vaccinated?] [set color green]
end
...
to-report Death-number
  report (init-population - count turtles)
end
to-report Proportion-immuned
  report (count turtles with [immune?]) / init-population
end
to-report left-population
...
to-report Max-infected-proportion
  report max-infected / init-population
end

to-report proportion-uninfected
...
"Susceptible" 1.0 0 -13791810 true "" "plot (count turtles with [not infected? and not immune?]) / init-population"
"Immune" 1.0 0 -13840069 true "" "plot (count turtles with [immune?]) / init-population"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot death-number"
